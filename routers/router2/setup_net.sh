...
router ospf
 ...
 network 192.168.32.0/19 area 0.0.0.0
 network 192.168.96.0/19 area 0.0.0.0
 network 192.168.160.0/19 area 0.0.0.0
line vty
EOF
cat >> /etc/quagga/zebra.conf << EOF
interface enp0s8
 ip address 192.168.32.254/19
 ipv6 nd suppress-ra
interface enp0s9
 ip address 192.168.96.2/19
 ipv6 nd suppress-ra
interface enp0s10
 ip address 192.168.160.2/19
 ipv6 nd suppress-ra
interface lo
ip forwarding
line vty
